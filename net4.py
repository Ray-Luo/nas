...
class SmartConv(nn.Module):
    ...
    def forward(self, x, padding=0):
        x, dilation, stride = self.smart_padding(x, self.target_height, self.target_width)

        adjusted_padding = (self.kernel_size - 1) * dilation // 2 + padding
        self.conv = nn.Conv2d(self.in_channels, self.out_channels, self.kernel_size, stride, padding=adjusted_padding, dilation=dilation, bias=True)
        x = self.conv(x)

        x, _, _ = self.smart_padding(x, self.target_height, self.target_width)
...
# net = SmartConv(target_height, target_width)
# out = net(a, 1, 1, 3)
# print(out)
...
for name, param in net.named_parameters():
    if name == "conv1.resolution_subsampling_weights":
        softmax = nn.functional.softmax(param, dim=0)
        parts = torch.tensor([16, 8, 4])
        sum = torch.sum(softmax * parts)
        print(name, param.size(), sum.item())
    if name == "conv2.resolution_subsampling_weights":
        softmax = nn.functional.softmax(param, dim=0)
        parts = torch.tensor([16, 8, 4])
        sum = torch.sum(softmax * parts)
        print(name, param.size(), sum.item())
    if name == "conv3.resolution_subsampling_weights":
        softmax = nn.functional.softmax(param, dim=0)
        parts = torch.tensor([16, 8, 4])
        sum = torch.sum(softmax * parts)
        print(name, param.size(), sum.item())
    if name == "conv1.conv_kernel_weights":
        softmax = nn.functional.softmax(param, dim=0)
        parts = torch.tensor([3,5])
        sum = torch.sum(softmax * parts)
        print(name, param.size(), sum.item())
    if name == "conv2.conv_kernel_weights":
        softmax = nn.functional.softmax(param, dim=0)
        parts = torch.tensor([3,5])
        sum = torch.sum(softmax * parts)
        print(name, param.size(), sum.item())
    if name == "conv3.conv_kernel_weights":
        softmax = nn.functional.softmax(param, dim=0)
        parts = torch.tensor([3,5])
        sum = torch.sum(softmax * parts)
        print(name, param.size(), sum.item())
    if name == "conv1.channel_mask.alpha":
        weights = nn.functional.softmax(param, dim=0)
        masks = nn.Parameter(torch.rand(3, 100))
        binary_masks = masks.round().clamp(0, 1)
        combined_mask = torch.sum(weights.view(-1, 1) * binary_masks, dim=0)
        print(name, param.size(), torch.sum(combined_mask).item())
    if name == "conv2.channel_mask.alpha":
        weights = nn.functional.softmax(param, dim=0)
        masks = nn.Parameter(torch.rand(3, 150))
        binary_masks = masks.round().clamp(0, 1)
        combined_mask = torch.sum(weights.view(-1, 1) * binary_masks, dim=0)
        print(name, param.size(), torch.sum(combined_mask).item())
    if name == "conv3.channel_mask.alpha":
        weights = nn.functional.softmax(param, dim=0)
        masks = nn.Parameter(torch.rand(3, 300))
        binary_masks = masks.round().clamp(0, 1)
        combined_mask = torch.sum(weights.view(-1, 1) * binary_masks, dim=0)
        print(name, param.size(), combined_mask)
